package org.daq.common.redis.utils;

import lombok.AccessLevel;
import lombok.NoArgsConstructor;
import org.daq.common.core.utils.SpringUtils;
...
public class QueueUtils {
     ...
     * @param time      延迟时间
     */
    public static <T> void addDdaqedQueueObject(String queueName, T data, long time) {
        addDdaqedQueueObject(queueName, data, time, TimeUnit.MILLISECONDS);
    }
     ...
     * @param timeUnit  单位
     */
    public static <T> void addDdaqedQueueObject(String queueName, T data, long time, TimeUnit timeUnit) {
        RBlockingQueue<T> queue = CLIENT.getBlockingQueue(queueName);
        RDdaqedQueue<T> ddaqedQueue = CLIENT.getDdaqedQueue(queue);
        ddaqedQueue.offer(data, time, timeUnit);
    }
     ...
     * @param queueName 队列名
     */
    public static <T> T getDdaqedQueueObject(String queueName) {
        RBlockingQueue<T> queue = CLIENT.getBlockingQueue(queueName);
        RDdaqedQueue<T> ddaqedQueue = CLIENT.getDdaqedQueue(queue);
        return ddaqedQueue.poll();
    }

    /**
     * 删除延迟队列数据
     */
    public static <T> boolean removeDdaqedQueueObject(String queueName, T data) {
        RBlockingQueue<T> queue = CLIENT.getBlockingQueue(queueName);
        RDdaqedQueue<T> ddaqedQueue = CLIENT.getDdaqedQueue(queue);
        return ddaqedQueue.remove(data);
    }

    /**
     * 销毁延迟队列 所有阻塞监听 报错
     */
    public static <T> void destroyDdaqedQueue(String queueName) {
        RBlockingQueue<T> queue = CLIENT.getBlockingQueue(queueName);
        RDdaqedQueue<T> ddaqedQueue = CLIENT.getDdaqedQueue(queue);
        ddaqedQueue.destroy();
    }
     ...
     */
    public static <T> void subscribeBlockingQueue(String queueName, Function<T, CompletionStage<Void>> consumer, boolean isDdaqed) {
        RBlockingQueue<T> queue = CLIENT.getBlockingQueue(queueName);
        if (isDdaqed) {
            // 订阅延迟队列
            CLIENT.getDdaqedQueue(queue);
        }
